// Lab 5 Halici, Alec Thursday 5:45pm - 10:00pm
#include <iostream>
#include <iomanip>
#include "Stack.h"
...
/*
void outputOrder(Stack& WidgetsOnHandFunc, Stack& OrdersToBeFilledFunc)
{
	// If Orders != empty and Widgets == empty
	
	if (!(OrdersToBeFilledFunc.isEmpty()) && WidgetsOnHandFunc.isEmpty())
	{
		cout << "\nOrder Number #" << right << setw(35) << OrdersToBeFilledFunc.getOrderNumber()
			<< "\nQuantity Ordered" << right << setw(33) << OrdersToBeFilledFunc.peek()->getStock()
			<< "\nQuantity Shipped this Shipment" << setw(19) << "0"
			<< "\nQuantity to be Shipped" << setw(27) << OrdersToBeFilledFunc.peek()->getStock()
			<< "\nTotal Cost to the Warehouse" << setw(22) << "N/A"
			<< "\nTotal Cost to the Customer" << setw(23) << "N/A"
			<< "\nProfit this Shipment" << setw(29) << "N/A" << endl;
	}
	...
	// If widgets can fulfill order
	if (WidgetsOnHandFunc.peek()->getStock() >= OrdersToBeFilledFunc.peek()->getStock())
	{
		cout << "\nOrder Number #" << right << setw(35) << OrdersToBeFilledFunc.getOrderNumber()
			<< "\nQuantity Ordered" << right << setw(33) << OrdersToBeFilledFunc.peek()->getStock()
			<< "\nQuantity Shipped this Shipment" << setw(19) << OrdersToBeFilledFunc.peek()->getStock()
			<< "\nQuantity to be Shipped" << setw(27) << "0"
			<< "\nTotal Cost to the Warehouse" << setw(22) << OrdersToBeFilledFunc.peek()->getStock() * WidgetsOnHandFunc.peek()->getPrice()
			<< "\nTotal Cost to the Customer" << setw(23) << OrdersToBeFilledFunc.peek()->getStock() * WidgetsOnHandFunc.peek()->getPrice() * 1.5
			<< "\nProfit this Shipment" << setw(29) << (OrdersToBeFilledFunc.peek()->getStock() * WidgetsOnHandFunc.peek()->getPrice() * 1.5) - OrdersToBeFilledFunc.peek()->getStock() * WidgetsOnHandFunc.peek()->getPrice() << endl;
	}
